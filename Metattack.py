...
class BaseMeta(BaseAttack):
    ...
    def __init__(self, model=None, nnodes=None, feature_shape=None, lambda_=0.5, attack_structure=fALSE, attack_features=False, undirected=True, device='cpu'):
...
class Metattack(BaseMeta):
    ...
    def __init__(self, model, nnodes, feature_shape=None, attack_structure=True, attack_features=False, undirected=True, device='cpu', with_bias=False, lambda_=0, train_iters=100, lr=0.1, momentum=0.9):
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        #     preds_node = F.nll_loss(output[idx], labels_self_training[idx])
        ...
        # plt.savefig('results_figure/cora_meta_l2.pdf',bbox_inches='tight', dpi=1000)
        # plt.show()
        if self.lambda_ == 1:
    ...
    def attack(self, ori_features, ori_adj, labels, idx_train, idx_unlabeled,idx_full, n_perturbations, ll_constraint=True, ll_cutoff=0.004,attack_type = None):
        ...
        for i in tqdm(range(n_perturbations), desc="Perturbing graph"):
            ...
            with torch.no_grad():
                ...
                if self.attack_structure:
                    ...
                    # priority = torch.norm(adj_meta_score, p=2, dim=1)
                    # csv_file = 'data.csv'
